...
void modularAdd(const uint8_t *first,
...
void modularAdd2(const uint8_t *first,
                const uint8_t *second,
                uint8_t *sum2,
                uECC_Curve curve) {
...
void modularAddULS(uECC_word_t *first,
                const uint8_t *second,
                uECC_word_t *sum,
                uECC_Curve curve) {
...
void modularSub(uECC_word_t *first,
                uECC_word_t *second,
                uECC_word_t *sum,
                uECC_Curve curve) {
...
void modularMult(const uint8_t *first,
                const uint8_t *second,
                uECC_word_t *mult,
                uECC_Curve curve) {
...
void modularMult2(const uint8_t *first,
                const uint8_t *second,
                uint8_t *mult2,
                uECC_Curve curve) {
...
void modularMultAdd(const uint8_t *mult1,
                	const uint8_t *mult2,
					const uint8_t *add,
                	uint8_t *res,
                	uECC_Curve curve) {
	...
	uECC_word_t resVLI[uECC_MAX_WORDS];
...
int uECC_make_private_key(uint8_t *private_key,
                  uECC_Curve curve) {
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
...
int uECC_shared_secret2(const uint8_t *public_key,
    ...
    EccPoint_mult(_public, _public, p2[!carry], initial_Z, curve->num_n_bits + 1, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) secret, (uint8_t *) _public, num_bytes * 2);
#else
    uECC_vli_nativeToBytes(secret, num_bytes, _public);
	uECC_vli_nativeToBytes(secret + num_bytes, num_bytes, _public + num_bytes);
#endif
    return !EccPoint_isZero(_public, curve);
}
...
void EllipticAdd(const uint8_t *first,
                 const uint8_t *second,
                 uint8_t *sum2,
                 uECC_Curve curve) {
    ...
    uECC_vli_bytesToNative(s2 + num_words, second + curve->num_bytes, curve->num_bytes);

    //uECC_vli_bytesToNative(sum, sum2, curve->num_bytes);
    ...
    uECC_vli_set(ty, s1 + num_words, num_words);

    //uECC_vli_set(tx2, s1, num_words);

    //uECC_vli_set(ty2, s1 + num_words, num_words);


    uECC_vli_modSub(z, sum, tx, curve->p, num_words); /* z = x2 - x1 */
    XYcZ_add(tx, ty, sum, sum + num_words, curve);
    uECC_vli_modInv(z, z, curve->p, num_words); /* z = 1/z */
    //uECC_vli_modMult_fast(z, tx2, ty,curve);
    //uECC_vli_modInv(z, z, curve->p,num_words);
    //uECC_vli_modMult_fast(z, z, tx,curve);
