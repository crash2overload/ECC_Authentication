...
void get_hashed_ID(element_t *ID, element_t publicKey, pairing_t pairing) {
	
	int n = element_length_in_bytes(publicKey);
	int m = element_length_in_bytes(*ID);
	unsigned char *data = pbc_malloc(n);
	unsigned char *hash = pbc_malloc(m);
		
	element_to_bytes(data, publicKey);
	
	blake2b_state state;
	blake2b_init(&state, m);
	blake2b_update(&state, data, n);
	blake2b_final(&state, hash, m);
	
	element_from_bytes(*ID, data);
...
int main(int argc, char **argv) {
	...
	/* e(G, U_A) ^ p_A */
	element_pow_zn(Alice.sharedKey, Alice.pairing, p);
	element_printf("k_E = %B\n", Alice.sharedKey);
	
	element_t ID_B;
	element_init_Zr(ID_B, pairing);
	get_hashed_ID(&ID_B, Bob.publicKey, pairing);
